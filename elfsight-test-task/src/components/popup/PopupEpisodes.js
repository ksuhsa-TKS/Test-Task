...
export function PopupEpisodes({ episodes }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(({ data }) => {
        // Убираем проверку на длину масcива, и меняем на проверку массива.
        // Так же если данных прерываем итерацию.
        if (!data) return;
        setSeries(Array.isArray(data) ? data : [data]);
      })
      // После выполнения всего дерева запросов убираем Loader.
  ...
  // Добавляем заглушку если данные отсутствуют.
  if (!series.length) {
